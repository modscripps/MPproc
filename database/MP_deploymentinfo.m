...
%             info.n_profiles=1415;   % stopped profiling after cast 220
  ...
  case 'FLEAT16' % FLEAT16
    switch sn
      ...
      case 'MM01'
        switch mooring
          case 'F1'
            info.lat = 8.6806; %surveyed
            info.lon = 134.4014;
            info.H=3390;
            info.dates={'6/2/2016-4/14/2017'};
            info.pmin=1527;
            info.pmax=2309;
            info.dt=9/24; %continuous
            
            info.n_profiles=39; % seems like it's only 39 good ones
            info.year=2016;
            info.start_yday=yearday(2,6,2016,0,0,0);
            info.end_yday=yearday(14,4,2016,0,0,0);
      ...
      case 'MM02'
        switch mooring
          case 'F1'
            ...
            info.CTDsensor=CTDsensorinfo_MP('SBE-MP52-026');%
            info.VELsensor=VELsensorinfo_MP('1230'); %
  ...
  case 'SR1614' % La Jolla Canyon SR1614
    switch sn
      case '105'
        switch mooring
          case 'MP1'
            info.lat = 32.857; %surveyed
            info.lon = -117.303;
            info.H=415;
            info.dates={'9/10/2016-12/15/2016'};
            info.pmin=20;
            info.pmax=405;
            info.dt=0; %continuous
            
            info.n_profiles=1361;
            info.year=2016;
            info.start_yday=yearday(10,9,2016,0,0,0);
            info.end_yday=yearday(15,12,2016,0,0,0);
            
            info.magvar=11.697992; % using magdec -117.303 32.857 2016 10 01
            
            info.CTDsensor=CTDsensorinfo_MP('SBE-MP52-123');%
            info.VELsensor=VELsensorinfo_MP('1869'); %
