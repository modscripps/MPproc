...
if nargin < 2
    ...
    infile.dlon=0.005;
    infile.dlat=0.005;
    infile.latm=0; %Use this to say that lat/lon are x/y
    infile.connectdots=1;
    pp.len_grid=30;
end
...
if ~isfield(infile,'latm')
    infile.latm=nanmean(CTDdata.lat);
end
if ~isfield(infile,'connectdots')
    infile.connectdots=0;
end
if ~isfield(infile,'len_grid')
    infile.len_grid=30;
end
...
for i = 1:length(infile.vars)
    ...
    if infile.connectdots==1
        h=plot(CTDdata.lon,CTDdata.lat,'k-');
        lc(h,0.7*[1 1 1])
    end
    ...
    title(['Cruise: ' infile.cruise, ', data group: ' int2str(infile.group) ...
