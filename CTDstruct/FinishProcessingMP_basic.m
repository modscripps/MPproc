...
if nargin < 2    
    ...
    PP.zvals=min(MP.z):max(MP.z); %1-m intervals
...
MP.uz=diffs(MP.u)/dz; 
MP.vz=diffs(MP.v)/dz;
MP.uz=CenteredConv(MP.uz,PP.ft,nv);
MP.vz=CenteredConv(MP.vz,PP.ft,nv);
shear=sqrt(MP.uz.^2+MP.vz.^2);
%MP.shearm=nanmean(MP.shear')';

MP.gregg=GreggHenyeyDissFCN(shear,sqrt(MP.n2m));
